...
class LazyBratsDataset(Dataset):
    def __init__(self, data_path, lazy_chunk_size=50):
        ...
        self.data = {}
        self.chunk_start = 0
        self.chunk_end = lazy_chunk_size if lazy_chunk_size > len(self.files[MASKS_IDX]) else len(self.files[MASKS_IDX])

        self._load_chunk()

    def _get_file_paths(self):
        volumes = sorted(glob.glob(self.data_path + VOLUMES_PATH + "/*.npy"))
        masks = sorted(glob.glob(self.data_path + MASKS_PATH + "/*.npy"))
